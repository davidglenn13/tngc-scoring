...
const snapshot=read("functions/api/v2/events/[id]/snapshot.js");
const ballApi=read("functions/api/v2/events/[id]/forty-ball.js");
const clientApi=read("src/core/api.js");
...
const pageOrder=["scorePane","gamesPane","ledgerPane","scorecardPane"].map(id=>html.indexOf(`data-pane="${id}"`));
if(pageOrder.some(x=>x<0)||pageOrder.some((x,i)=>i&&x<=pageOrder[i-1])||html.indexOf('id="quickGameSetup"')<html.indexOf('id="gamesPane"'))fail("Ballyhack page order and game setup placement");
...
if(!app.includes("showJoinGate")||!app.includes("lockedViewer")||!app.includes("private Foursome")||!app.includes("Opponent scoring is private"))fail("shared-link foursome privacy UI");
if(!common.includes("teamAccessToken")||!common.includes("requireEventAccess")||!snapshot.includes("p.foursome_no=?")||!scores.includes("access.foursome")||!ballApi.includes("access.foursome")||!clientApi.includes("x-team-access"))fail("server-enforced private foursome access");
...
if(!app.includes("fortyWager:''")||!app.includes("Enter the ${ballName()} wager before continuing")||app.includes("fortyWager:5"))fail("empty required Ball wager");
if(!app.includes("state.games.stableford&&net!==null")||!app.includes("Gross <strong>")||!app.includes("netLabel=state.games.forty?'Net to par':'Net'"))fail("explicit gross net and optional Stableford scoring");
if(!app.includes("netLabel=state.games.forty?'Net to par':'Net'")||!app.includes("formatRelative(net-course.par[h-1])"))fail("Ball net relative to par");
...
if(html.indexOf('id="fortyBallTracker"')>html.indexOf('class="hole-nav"'))fail("Ball tracker must be above score entry navigation");
